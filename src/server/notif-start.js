...
function monitor(iCPULoadData)
{
  if(iCPULoadData.loadAverage > exceedMax){
    ...
    if(counterExceed === totalTimes){
      ...
      console.log("it started exceed from for the " + (monitoringResult.exceed.length+1) + "th times, begins at date: "+ iCPULoadData.timeUTC);
    }
  }else{
    if(counterExceed >= totalTimes)
    {
      ...
      console.log("it started recovery from for the " + (monitoringResult.recovery.length+1) + "th times, begins at date: "+ iCPULoadData.timeUTC);
    }
